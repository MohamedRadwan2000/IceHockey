...
public class icehockey implements IPlayersFinder{
    ...
    public java.awt.Point[] findPlayers( String[] photo, int team, int threshold){
        ...
        for(int i=0;i<counter1;i++){
            if((right[i].x==-1)&&(right[i].y==-1)){continue;}
            Point each_node[]=new Point[right.length];
            for(int k=0;k<right.length;k++){each_node[k]=new Point(-1,-1);}
            ...
            for(int j=1;j<counter2;j++){
                for(int k=0;k<counter1;k++){
                    ...
                    if(dif==1){
                        ...
                        if(n){each_node[counter2].setLocation(right[k].x,right[k].y);
                        counter2++;}
                    }
                }
            }
            if((counter2*4)>=(threshold)){
             ...
             for(int m=0;m<counter2;m++){
                 if(each_node[m].x>x_max){x_max=each_node[m].x;}
                 if(each_node[m].x<x_min){x_min=each_node[m].x;}
                 if(each_node[m].y<y_min){y_min=each_node[m].y;}
                 if(each_node[m].y>y_max){y_max=each_node[m].y;}
             }
        ...
        for(int k=0;k<counterpixel-1;k++){
            for(int l=0;l<counterpixel-k-1;l++){
                if(pixel2[l].x>pixel2[l+1].x){Point temp=new Point(pixel2[l]);
                pixel2[l]=pixel2[l+1];
                pixel2[l+1]=temp;}
